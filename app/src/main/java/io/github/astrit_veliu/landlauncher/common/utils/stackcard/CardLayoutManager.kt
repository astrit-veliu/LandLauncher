...
class CardLayoutManager : LayoutManager, ScrollVectorProvider {
    ...
    interface ViewUpdater {
         ...
         */
        fun onLayoutManagerInitialized(lm: CardLayoutManager)
    ...
    override fun onLayoutChildren(recycler: Recycler, state: State) {
        ...
        if (state.isPreLayout) {
            ...
            while (i < cnt) {
                ...
                val isRemoved = (child!!.layoutParams as LayoutParams).isItemRemoved
                if (isRemoved) {
    ...
    private fun loadViewUpdater(context: Context, className: String?, attrs: AttributeSet): ViewUpdater? {
        ...
        if (className[0] == '.') {
            ...
            fullClassName = CardLayoutManager::class.java.getPackage().name + '.' + className
        }
    ...
    private fun scrollRight(dx: Int): Int {
        ...
        for (view in rightViews) {
            view?.let {
                val border = activeCardLeft + getPosition(it) * cardWidth
                val allowedDelta = getAllowedRightDelta(it, dx, border)
                it.offsetLeftAndRight(-allowedDelta)
            }
        }
    ...
    private fun fill(anchorPos: Int, recycler: Recycler, state: State) {
        viewCache.clear()
        run {
            var i: Int = 0
            val cnt: Int = getChildCount()
            while (i < cnt) {
